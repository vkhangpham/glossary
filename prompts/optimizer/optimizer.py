...
def optimize_prompt(
    ...
    metrics = {
        ...
        "num_iterations": len(result.iteration_scores) if hasattr(result, 'iteration_scores') else 0,
        ...
        "improvement": (result.best_score - result.seed_score) if hasattr(result, 'best_score') and hasattr(result, 'seed_score') else 0
    }
    
    logger.info(f"Optimization complete in {elapsed_time:.2f}s")
    logger.info(f"Score improved from {metrics['initial_score']:.3f} to {metrics['final_score']:.3f}")
...
def compare_prompts(
    ...
    original_eval = adapter.evaluate(test_data, original_candidate, capture_traces=False)
    ...
    optimized_eval = adapter.evaluate(test_data, optimized_candidate, capture_traces=False)
