...
class TokenUsage:
    ...
    Attributes:
        ...
        reasoning_tokens: Number of tokens used for reasoning (only available for OpenAI's o3-mini model)
            This is a special field that's only populated when using OpenAI's o3-mini model.
...
class TokenTracker:
    ...
    def calculate_openai_cost(prompt_tokens: int, completion_tokens: int, model: str) -> float:
        ...
        # Only support o3-mini, gpt-4o, and deepseek-chat models
        if model == "o3-mini":
            ...
            raise ValueError(f"Unsupported OpenAI model for cost calculation: {model}. Only o3-mini, gpt-4o, and deepseek-chat are supported.")
