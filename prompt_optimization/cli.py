...
@click.command()
@click.option('--prompt', required=True, help='Prompt to optimize (e.g., lv0_s1, lv1_s2)')
@click.option('--auto', default='medium', type=click.Choice(['light', 'medium', 'heavy']), 
...
@click.option('--task-model', default='openai/gpt-5-nano', help='Model for task execution')
@click.option('--reflection-model', default='openai/gpt-5', help='Model for reflection/optimization')
@click.option('--train-split', default=0.7, type=float, help='Train/validation split ratio')
@click.option('--batch-size', default=5, type=int, help='Training batch size (production uses 20)')
...
def optimize_prompt(
    ...
    try:
        ...
        click.echo(f"Expected file: prompt_optimization/optimizers/{prompt}.py", err=True)
        
        optimizers_dir = Path("prompt_optimization/optimizers")
        available = [f.stem for f in optimizers_dir.glob("*.py") 
                    if f.stem not in ['__init__', 'template']]
        if available:
            click.echo(f"\nAvailable optimizers: {', '.join(available)}")
        return 1
    
    required_funcs = ['create_training_data', 'prepare_dspy_examples', 
                      'metric_with_feedback', 'get_program']
    missing = [f for f in required_funcs if not hasattr(optimizer_module, f)]
    if missing:
        click.echo(f"Error: Optimizer {prompt} is missing required functions: {missing}", err=True)
        click.echo("Please ensure the optimizer follows the template structure", err=True)
        return 1
    ...
    if "gpt-5" in task_model:
        task_lm = dspy.LM(task_model, api_key=api_key, temperature=1.0, max_tokens=16000)
    else:
        task_lm = dspy.LM(task_model, api_key=api_key, temperature=1.0)
    
    if "gpt-5" in reflection_model:
        reflection_lm = dspy.LM(reflection_model, api_key=api_key, temperature=1.0, max_tokens=16000)
    else:
    ...
    click.echo(f"Using {len(trainset)} training examples and {len(valset)} validation examples\n")
    ...
    if hasattr(optimized_program, "detailed_results"):
        ...
        if hasattr(stats, 'get'):
            ...
            click.echo(f"  - Candidates evaluated: {stats.get('candidates_evaluated', 'N/A')}")
