...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("terms_file", help="Path to a file containing terms to mine (one per line)")
    parser.add_argument("-o", "--output", required=True, help="Output path (file or directory)")
    parser.add_argument("-c", "--max-concurrent", type=int, help="Maximum concurrent operations")
    parser.add_argument("-b", "--batch-size", type=int, help="Number of terms per batch")
    parser.add_argument("--max-urls", type=int, help="Maximum URLs to fetch per concept")
    parser.add_argument("--max-age", type=int, help="Cache duration in milliseconds")
    parser.add_argument("--max-pages", type=int, help="Maximum pages to extract per document")
    parser.add_argument(
        "--no-batch",
        dest="use_batch",
        action="store_false",
    ...
    parser.add_argument("-q", "--quiet", action="store_true", help="Silence non-error logs")
    parser.add_argument("--output-format", choices=["json", "jsonl", "csv"], dest="output_format", help="Output format")
    ...
    parser.add_argument("--include-source-urls", dest="include_source_urls", action="store_true")
    parser.add_argument("--no-include-source-urls", dest="include_source_urls", action="store_false")

    parser.set_defaults(save_metadata=None, include_source_urls=None)
    return parser


def apply_logging_overrides(config: MiningModuleConfig, args: argparse.Namespace) -> None:
    if args.verbose:
...
def mine_concepts_simple(
    ...
    for index, term in enumerate(term_list, start=1):
        ...
        academic_urls = filter_academic_urls(urls) or urls
        ...
        scrape_params = prepare_firecrawl_params(mining_config, **extra_kwargs)

        if mining_config.use_batch_scrape and hasattr(firecrawl_client, "batch_scrape"):
            op_start = time.time()
            try:
                response = firecrawl_client.batch_scrape(urls=academic_urls, **scrape_params)
                track_mining_metrics("batch_scrape", time.time() - op_start, True)
            except Exception as exc:  # pragma: no cover - depends on Firecrawl SDK
                track_mining_metrics("batch_scrape", time.time() - op_start, False)
                errors.append(handle_firecrawl_error(exc, {"operation": "batch_scrape", "term": term}))
                continue
            ...
            for url in academic_urls:
                ...
                try:
                    ...
                    errors.append(handle_firecrawl_error(exc, {"operation": "scrape", "term": term, "url": url}))
                    continue
                extracted.extend(handle_batch_scrape_response(response))

        for record in extracted:
            record["concept"] = term
            record.setdefault("score", score_academic_relevance(record.get("content", "") or "", term))
...
def monitor_job_if_requested(client: Any, job_id: Optional[str]) -> Optional[Dict[str, Any]]:
    if not job_id:
        return None
    try:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    setup_mining_logger(config.logging)
    ...
    start = time.time()
    aggregated = mine_concepts_simple(terms, client, config.mining, webhook_url=args.webhook_url)
    ...
    with open(Path(output_file).with_suffix(Path(output_file).suffix + ".meta.json"), "w", encoding="utf-8") as meta_file:
        json.dump(aggregated, meta_file, indent=2, sort_keys=True)

    logging.getLogger(__name__).info(
        "Mining complete | %s", format_progress_message(len(terms), len(terms), duration)
    )
    return 0
