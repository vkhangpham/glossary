...
def get_queue_status(app: Optional[Any] = None) -> Optional[QueueStatus]:
    ...
    Args:
        app: Initialized Firecrawl client, will get default if None

    Returns:
        QueueStatus object with enhanced metrics or None if failed
    """
    with processing_context("get_queue_status") as correlation_id:
        app = _resolve_firecrawl_client(app, "get_queue_status", correlation_id)
        if app is None:
            return None
        ...
        try:
            ...
            if isinstance(status_response, dict):
                ...
                queue_status = QueueStatus(
                    ...
                    queue_utilization=status_response.get(
                        "queue_utilization", queue_utilization
                    ),
                    estimated_wait_time=status_response.get(
                ...
                log_processing_step(
                    ...
                    "completed",
                    {
                        ...
                        "queue_utilization": queue_status.waiting_jobs
                        / max(1, queue_status.max_concurrency),
                        "predicted_completion_time": queue_insights.get(
                            "predicted_completion_time", 0
                        ),
                        "recommended_concurrency": queue_insights.get(
                            "recommended_concurrency", queue_status.max_concurrency
                        ),
            ...
            handle_error(
                ExternalServiceError(
                    f"Enhanced queue status request failed: {e}", service="firecrawl"
                ),
...
async def get_queue_status_async(app: Optional[Any] = None) -> Optional[QueueStatus]:
    """Async wrapper for get_queue_status that runs in a thread to avoid blocking.

    Args:
        app: Initialized Firecrawl client

    Returns:
        QueueStatus object with enhanced metrics or None if failed
    """
    resolved_client = _resolve_firecrawl_client(app, "get_queue_status_async")
    if resolved_client is None:
        return None

    return await asyncio.to_thread(get_queue_status, resolved_client)
...
def generate_queue_insights(queue_status: QueueStatus) -> Dict[str, Any]:
    ...
    try:
        ...
        recommended_concurrency = _recommend_optimal_concurrency(
            queue_status.max_concurrency
        )
        ...
        if len(_queue_predictor.queue_history) >= 3:
            recent_loads = [
                s["total_jobs"] for s in list(_queue_predictor.queue_history)[-3:]
            ]
            trend = (
                "increasing"
                if recent_loads[-1] > recent_loads[0]
                else "decreasing" if recent_loads[-1] < recent_loads[0] else "stable"
            )
...
def check_queue_health(queue_status: QueueStatus) -> List[str]:
    ...
    try:
        ...
        if utilization > 2.0:
            alerts.append(
                f"CRITICAL: Queue severely overloaded ({utilization:.1f}x capacity)"
            )
        elif utilization > 1.5:
            alerts.append(
                f"HIGH: Queue significantly overloaded ({utilization:.1f}x capacity)"
            )
        elif utilization > 1.0:
        ...
        if len(_queue_predictor.queue_history) >= 5:
            ...
            if all(s["active_jobs"] == 0 for s in recent_statuses) and any(
                s["waiting_jobs"] > 0 for s in recent_statuses
            ):
                alerts.append(
                    "CRITICAL: Queue appears stagnant - no active jobs despite waiting jobs"
                )
        ...
        if len(_queue_predictor.completion_times) >= 10:
            ...
            if len(older_times) >= 3 and len(recent_times) >= 3:
                ...
                if avg_recent > avg_older * 1.5:
                    ...
                    alerts.append(
                        f"MEDIUM: Performance degradation detected "
        ...
        if recommended_concurrency != queue_status.max_concurrency:
            ...
            alerts.append(
                f"INFO: Consider {direction} concurrency to {recommended_concurrency} "
...
async def poll_job_with_adaptive_strategy(
    app: Any, job_id: str, enable_queue_monitoring: bool = False
) -> Dict[str, Any]:
        ...
        try:
            ...
            if enable_queue_monitoring:
                ...
                if queue_status:
                    ...
                    predicted_timeout = max(
                        predicted_time * 1.5, 120.0
                    )  # 50% buffer, minimum 2 minutes
                    max_wait_seconds = min(
                        predicted_timeout, 900.0
                    )  # Maximum 15 minutes
            ...
            while attempt < max_attempts:
                ...
                try:
                    ...
                    try:
                        ...
                        status = getattr(
                            app, "check_crawl_status", lambda x: {"status": "unknown"}
                        )(job_id)
            ...
            final_error = (
                last_error or f"Polling timed out after {max_attempts} attempts"
            )
            log_with_context(
                logger,
                logging.ERROR,
...
async def apply_intelligent_throttling(
    app: Optional[Any] = None, current_profile=None
) -> float:
    ...
    with processing_context("intelligent_throttling") as correlation_id:
        app = _resolve_firecrawl_client(app, "intelligent_throttling", correlation_id)
        if app is None:
            return 0.0
        ...
        try:
            ...
            queue_utilization = current_queue_status.waiting_jobs / max(
                1, current_queue_status.max_concurrency
            )
            total_load = (
                current_queue_status.active_jobs + current_queue_status.waiting_jobs
            )
