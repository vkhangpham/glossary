        ...
        **batch_func_kwargs,
        ...
        if not force_restart:
            ...
            if can_resume:
                ...
                logger.info(
                    f"Resuming from checkpoint: {resume_info['progress']['percentage']:.1f}% complete"
                )
        ...
        for batch_idx in tqdm(range(num_batches), desc=f"Processing {self.operation}"):
            ...
            try:
                logger.info(
                    f"Processing batch {batch_idx + 1}/{num_batches} ({len(batch_items)} items)"
                )

                # Process the batch
                batch_results = process_batch_func(batch_items, **batch_func_kwargs)
                ...
                if (
                    batch_idx + 1
                ) % cooldown_frequency == 0 and batch_idx < num_batches - 1:
    ...
    def process_sources(
        ...
        def process_batch(chunks_batch):
            ...
            for chunk in chunks_batch:
                try:
                    chunk_results = process_chunk_func((chunk, llm_attempts))
