...
def validate_step_dependencies(
    level: int, step: int, input_file: Optional[str] = None, create_dirs: bool = False
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            if input_file:
                ...
                if not Path(input_file).exists():
                    ...
                    validation_result["errors"].append(
                        f"Input file override not found: {input_file}"
                    )
            else:
                ...
                if not Path(config_input_file).exists():
                    validation_result["valid"] = False
                    validation_result["errors"].append(
                        f"Input file not found: {config_input_file}"
                    )
            ...
            if level == 0 and step == 0:
                if input_file is None:
                    ...
                    validation_result["valid"] = False
                    validation_result["errors"].append(
                        f"Level 0 Step 0 requires Excel input file or input file override: {str(e)}"
                    )
                validation_result["warnings"].append(
                    f"Level 0 Step 0 uses Excel parsing, not web extraction: {str(e)}"
                )
            else:
                validation_result["valid"] = False
        ...
        if step > 0:
            ...
            if not Path(prev_output_file).exists():
                validation_result["valid"] = False
                validation_result["errors"].append(
                    f"Previous step output not found: {prev_output_file}"
                )
        ...
        if not output_dir.exists():
            if create_dirs:
                ...
                validation_result["warnings"].append(
                    f"Output directory needs creation: {output_dir}"
                )
...
def create_generic_step_runner(level: int, step: int) -> Callable:
    ...
    def run_step(**kwargs):
        ...
        validation = validate_step_dependencies(
            level, step, input_file=input_file_override, create_dirs=True
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to discover step function for Level {level} Step {step}: {str(e)}"
            )
            return 1

        # Determine test mode
        test_mode = (
            kwargs.get("test", False) or "--test" in sys.argv or "-t" in sys.argv
        )
        ...
        try:
            ...
            logger.error(
                f"Step execution failed for Level {level} Step {step}: {str(e)}"
            )
            return 1
