...
Answer with only "YES" or "NO" followed by a brief justification."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not response:
                logger.warning(f"Empty response for term '{term}' (attempt {attempt + 1})")
                continue
            ...
            if response_lower.startswith('yes') or 'yes,' in response_lower or 'yes.' in response_lower:
                ...
                if any(positive in response_lower for positive in ['valid', 'legitimate', 'appropriate', 'accepted']):
                    return True
                elif any(negative in response_lower for negative in ['invalid', 'not valid', 'inappropriate', 'rejected']):
                    return False
                else:
                    logger.warning(f"Unclear response for term '{term}': {response[:100]}...")
                    # Default to not valid for unclear responses
                    return False
                    
        except Exception as e:
            logger.warning(f"Error verifying term '{term}' (attempt {attempt + 1}): {str(e)}")
            if attempt == max_retries - 1:
                logger.error(f"Failed to verify term '{term}' after {max_retries} attempts")
...
def verify_single_tokens(
    ...
    logger.info(f"Starting Level {level} token verification: {config.processing_description}")
    ...
    logger.info(f"Single-word terms: {len(single_word_terms)}, Multi-word terms: {len(multi_word_terms)}")
    ...
    verification_stats = {
        ...
        'single_word_rejected_count': len(single_word_terms) - len(verified_single_word_terms),
        'verification_time_seconds': verification_time,
        'provider_used': provider or 'random_selection',
        'verification_rate': len(verified_single_word_terms) / len(single_word_terms) if single_word_terms else 0
    }
    
    # Save results
    save_verification_results(
        verified_single_word_terms, multi_word_terms, 
        output_file, metadata_file, level, verification_stats
    )
