...
def build_verification_prompt(
    ...
    Args:
        ...
        institutions: List of institutions where this concept appears
...
Example Institutions that mention this concept:
{institutions_str}
...
def verify_keyword(
    ...
    """
    # Skip if no institutions found
    if not institutions:
        logger.debug(f"No institutions found for keyword '{keyword}'")
        return False
...
def verify_keywords_batch(
    ...
    Args:
        ...
        concept_institutions: Dictionary mapping keywords to their institutions
        provider: Optional LLM provider (openai or gemini)
    ...
    for i in tqdm(range(0, len(keywords), batch_size), desc="Verifying keywords"):
        ...
        for keyword in batch:
            try:
                institutions = concept_institutions.get(keyword, [])
                is_verified = verify_keyword(keyword, institutions, provider)
...
def get_concept_institutions(metadata: Dict[str, Any]) -> Dict[str, List[str]]:
    """
    Extract institutions for each concept from metadata
    
    Args:
        metadata: Metadata from s1
        
    Returns:
        Dictionary mapping concepts to their institutions
    """
    ...
    for source, concepts in source_mapping.items():
        ...
        for concept in concepts:
            if concept not in concept_institutions:
                concept_institutions[concept] = []
            if source not in concept_institutions[concept]:
                concept_institutions[concept].append(source)
    
    return concept_institutions
