#!/usr/bin/env python3
"""
Level 2 Research Areas Generation Runner

This module provides CLI entry points for Level 2 (research areas) generation,
using the shared functional utilities configured for research area extraction.
"""
...
def run_step_0(input_file: str) -> dict:
    """Run Level 2 Step 0: Research area extraction from department terms."""
    logger.info(f"Running Level {LEVEL} Step 0: Research area extraction")
    
    # File paths for Level 2 Step 0
    output_file = f"data/lv{LEVEL}/raw/lv{LEVEL}_s0_research_areas.txt"
...
def run_step_1(provider: Optional[str] = None) -> dict:
    """Run Level 2 Step 1: Concept extraction from research areas."""
    logger.info(f"Running Level {LEVEL} Step 1: Concept extraction")
    
    # File paths for Level 2 Step 1
    input_file = f"data/lv{LEVEL}/raw/lv{LEVEL}_s0_research_areas.txt"
...
def run_step_2() -> dict:
    """Run Level 2 Step 2: Frequency filtering."""
    logger.info(f"Running Level {LEVEL} Step 2: Frequency filtering")
    
    # File paths for Level 2 Step 2
...
def run_step_3(provider: Optional[str] = None) -> dict:
    """Run Level 2 Step 3: Single token verification."""
    logger.info(f"Running Level {LEVEL} Step 3: Token verification")
    
    # File paths for Level 2 Step 3
...
def lv2_s0_main():
    """Console script entry point for Level 2 Step 0."""
    parser = argparse.ArgumentParser(description="Level 2 Step 0: Research area extraction")
    parser.add_argument("--input", 
                       help="Input file path (default: data/lv1/lv1_final.txt)",
                       default="data/lv1/lv1_final.txt")
...
def lv2_s1_main():
    """Console script entry point for Level 2 Step 1."""
    parser = argparse.ArgumentParser(description="Level 2 Step 1: Concept extraction")
...
def lv2_s2_main():
    """Console script entry point for Level 2 Step 2."""
    parser = argparse.ArgumentParser(description="Level 2 Step 2: Frequency filtering")
...
def lv2_s3_main():
    """Console script entry point for Level 2 Step 3."""
    parser = argparse.ArgumentParser(description="Level 2 Step 3: Token verification")
    parser.add_argument("--provider", 
                       choices=["openai", "gemini"],
                       help="LLM provider (default: random selection)")
...
def main():
    """Main CLI interface for Level 2 runner."""
    parser = argparse.ArgumentParser(description="Level 2 Research Areas Generation Runner")
    ...
    parser.add_argument("--input", 
                       help="Input file for step 0 (default: data/lv1/lv1_final.txt)")
    
    args = parser.parse_args()
    
    try:
        if args.step == "s0":
            input_file = args.input or "data/lv1/lv1_final.txt"
