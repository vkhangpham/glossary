#!/usr/bin/env python3
"""
Level 3 Conference Topics Generation Runner

This module provides CLI entry points for Level 3 (conference topics) generation,
using the shared functional utilities configured for conference topic extraction.
"""
...
def run_step_0(input_file: str) -> dict:
    """Run Level 3 Step 0: Conference topic extraction from research areas."""
    logger.info(f"Running Level {LEVEL} Step 0: Conference topic extraction")
    
    # File paths for Level 3 Step 0
    output_file = f"data/lv{LEVEL}/raw/lv{LEVEL}_s0_conference_topics.txt"
...
def run_step_1(provider: Optional[str] = None) -> dict:
    """Run Level 3 Step 1: Concept extraction from conference topics."""
    logger.info(f"Running Level {LEVEL} Step 1: Concept extraction")
    
    # File paths for Level 3 Step 1
    input_file = f"data/lv{LEVEL}/raw/lv{LEVEL}_s0_conference_topics.txt"
    ...
    # Use shared concept extraction utility
...
def run_step_2() -> dict:
    """Run Level 3 Step 2: Frequency filtering."""
    logger.info(f"Running Level {LEVEL} Step 2: Frequency filtering")
    
    # File paths for Level 3 Step 2
...
def run_step_3(provider: Optional[str] = None) -> dict:
    """Run Level 3 Step 3: Single token verification."""
    logger.info(f"Running Level {LEVEL} Step 3: Token verification")
    
    # File paths for Level 3 Step 3
...
def lv3_s0_main():
    """Console script entry point for Level 3 Step 0."""
    parser = argparse.ArgumentParser(description="Level 3 Step 0: Conference topic extraction")
    parser.add_argument("--input", 
                       help="Input file path (default: data/lv2/lv2_final.txt)",
                       default="data/lv2/lv2_final.txt")
...
def lv3_s1_main():
    """Console script entry point for Level 3 Step 1."""
    parser = argparse.ArgumentParser(description="Level 3 Step 1: Concept extraction")
...
def lv3_s2_main():
    """Console script entry point for Level 3 Step 2."""
    parser = argparse.ArgumentParser(description="Level 3 Step 2: Venue frequency filtering")
...
def lv3_s3_main():
    """Console script entry point for Level 3 Step 3."""
    parser = argparse.ArgumentParser(description="Level 3 Step 3: Token verification")
    parser.add_argument("--provider", 
                       choices=["openai", "gemini"],
                       help="LLM provider (default: random selection)")
...
def main():
    """Main CLI interface for Level 3 runner."""
    parser = argparse.ArgumentParser(description="Level 3 Conference Topics Generation Runner")
    ...
    parser.add_argument("--input", 
                       help="Input file for step 0 (default: data/lv2/lv2_final.txt)")
    
    args = parser.parse_args()
    
    try:
        if args.step == "s0":
            input_file = args.input or "data/lv2/lv2_final.txt"
